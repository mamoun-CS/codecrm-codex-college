...
export class UnifyLeads1700000000005 implements MigrationInterface {
  name = 'UnifyLeads1700000000005';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE leads ALTER COLUMN custom_fields TYPE jsonb USING COALESCE(custom_fields, '{}'::json)::jsonb`);
    ...
    await queryRunner.query(`ALTER TABLE leads ADD CONSTRAINT fk_leads_pipeline FOREIGN KEY (pipeline_id) REFERENCES pipelines(id) ON DELETE SET NULL`);
    ...
    await queryRunner.query(`ALTER TABLE lead_touchpoints ADD CONSTRAINT fk_lead_touchpoints_lead FOREIGN KEY (lead_id) REFERENCES leads(id) ON DELETE SET NULL`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Note: This is a complex migration that unifies data structures.
    // A full rollback would require recreating the original tables and moving data back.
    // For safety, we'll implement a minimal rollback that restores the original schema structure.

    // Restore original table structure (without data migration back)
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS source`);
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS source_reference_id`);
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS raw_payload`);
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS original_created_at`);
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS ingested_at`);
    await queryRunner.query(`ALTER TABLE leads DROP COLUMN IF EXISTS pipeline_id`);

    // Drop indexes
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_source_reference`);
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_source_created_at`);
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_owner_created_at`);
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_team_created_at`);
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_campaign_created_at`);
    await queryRunner.query(`DROP INDEX IF EXISTS idx_leads_ingested_at`);

    // Restore original constraints
    await queryRunner.query(`ALTER TABLE deals ALTER COLUMN lead_id SET NOT NULL`);
    await queryRunner.query(`ALTER TABLE activities ALTER COLUMN lead_id SET NOT NULL`);
    await queryRunner.query(`ALTER TABLE messages ALTER COLUMN lead_id SET NOT NULL`);

    // Note: Archived tables (meta_leads_archive, tiktok_leads_archive) are preserved for data recovery
  }
}
